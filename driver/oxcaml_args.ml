...
let mk_fdo_labels f =
  ( "-fdo-labels",
    Arg.Unit f,
    " Create pseudo-instrumentation labels for branching constructs and\n\
    \     emit the FDO metadata section (implied by -fdo-profile)" )

let mk_fdo_layout f =
  ( "-fdo-layout",
    Arg.Symbol ([ "greedy"; "ext-tsp" ], f),
    "  Block layout algorithm for -fdo-profile (default: ext-tsp)" )

let mk_fdo_names f =
  ( "-fdo-names",
    Arg.Unit f,
    " Also record the names of the locations in the FDO metadata section,\n\
...
module type Oxcaml_options = sig
  ...
  val fdo_labels : unit -> unit
  val fdo_names : unit -> unit
  val fdo_layout : string -> unit
...
module Make_oxcaml_options (F : Oxcaml_options) = struct
  let list2 =
    [
      ...
      mk_fdo_labels F.fdo_labels;
      mk_fdo_names F.fdo_names;
      mk_fdo_layout F.fdo_layout;
...
module Oxcaml_options_impl = struct
  ...
  let fdo_labels = set' Oxcaml_flags.fdo_labels
  let fdo_names = set' Oxcaml_flags.fdo_names

  let fdo_layout s =
    Oxcaml_flags.fdo_layout := Oxcaml_flags.fdo_layout_of_string s
...
module Extra_params = struct
  let read_param ppf _position name v =
    ...
    | "fdo-profile" ->
        ...
        true
    | "fdo-labels" -> set' Oxcaml_flags.fdo_labels
    | "fdo-names" -> set' Oxcaml_flags.fdo_names
    | "fdo-layout" ->
        Oxcaml_flags.fdo_layout := Oxcaml_flags.fdo_layout_of_string v;
        true
