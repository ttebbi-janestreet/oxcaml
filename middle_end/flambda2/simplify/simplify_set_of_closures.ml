...
let dacc_inside_function context ~outer_dacc ~params ~my_closure ~my_alloc_mode
  ...
  (* The debuginfo in the body of a copied code binding (see
     [DE.enter_set_of_closures]) must be rewritten exactly once: when a
     function is simplified again ([should_resimplify], below), its body
     already carries the rewritten debuginfo. *)
  let denv =
...
let simplify_function0 context ~outer_dacc function_slot_opt code_id code
  ...
  let code_const, new_code =
    (* CR vlaviron: Ideally the debuginfo should be associated to the set of
       ...
       allocation. *)
    let dbg =
      (* As for the body (see [dacc_inside_function]), rewrite only on the
         first simplification: on resimplification [Code.dbg code] has
