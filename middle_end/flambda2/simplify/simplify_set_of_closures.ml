...
let dacc_inside_function context ~outer_dacc ~params ~my_closure ~my_alloc_mode
  ...
  (* The labels in the body of a copied code binding (see
  ...
  let code_const, new_code =
    ...
    let dbg =
      ...
      then Code.dbg code
        ...
        DE.add_inlined_debuginfo_to_code_binding (DA.denv outer_dacc)
          (Code.dbg code)
    in
    let dbg =
      (* The labels of the calls inlined at the head of the body ride on the
         entry edge (see [Inlined_call_labels]). *)
      let denv = DA.denv outer_dacc in
      if DE.tracking_inlined_call_labels denv
      then
        Debuginfo.add_riders dbg ~position:0
          (Inlined_call_labels.labels_into
             (DE.inlined_call_labels denv)
             (Function_entry code_id))
      else dbg
    in
