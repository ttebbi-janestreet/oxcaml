...
let define_continuations ~can_be_lifted t conts =
  let replay_history =
    Replay_history.define_continuations ~can_be_lifted conts t.replay_history
  in
...
let enter_set_of_closures
  ...
  { machine_width;
    ...
    (* A code binding encountered while simplifying an inlined body is a copy of
       ...
       (e.g. the functions of a functor whose application is inlined out).
       Keeping [inlined_debuginfo] makes the debuginfo in such copies record the
       instantiation site, exactly as for the rest of the inlined body; in
       particular their pseudo-instrumentation labels get distinct creator
       stacks and discriminators, so that profile counts are not conflated
       between copies. (This does not affect line tables or
       [Debuginfo.to_location], which use the innermost frame.) The rewrite must
       be applied only on the first simplification of a code binding; see
