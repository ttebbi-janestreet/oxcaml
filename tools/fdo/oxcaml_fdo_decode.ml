(* Decodes a Linux perf profile of an OxCaml-compiled executable into a
   ...
   Profiles recorded with LBR branch stacks (-j any,u) are preferred: both
   addresses of every recorded taken branch become a sample, giving precise
   basic-block coverage. Plain instruction profiles (perf record without -j) are
   also supported.
...
let args =
  [ ( "-perf-data",
    ...
    ( "-perf-script-output",
      ...
      "<file>  read pre-captured 'perf script -F brstack,period' (or\n\
      \     'ip,dso,period') output instead of running perf" );
    ( "-dump",
      Arg.Set_string dump,
...
let decode () =
  ...
  (* Perf samples record runtime addresses, which match the link-time addresses
     the DWARF describes only for position-dependent executables. *)
  ...
  then
    Printf.eprintf
      ...
      !binary;
  let code_bounds = Elf_info.code_bounds elf in
  (* The executable's branch-label metadata, if it was compiled with labels
     (-fdo-labels or -fdo-profile): the branch addresses to collect outcome
     counts for, and the labels to attribute them to. *)
  let branch_labels =
    match Elf_info.section_body elf "fdo_branch_labels" with
    | Some data -> Branch_labels.parse data
    | None -> Hashtbl.create 0
  in
  let branch_sites =
    Hashtbl.fold
      (fun address (site : Branch_labels.site) acc ->
        { Perf_script.address; fallthrough_address = site.fallthrough_address }
        :: acc)
      branch_labels []
    |> Array.of_list
  in
  let samples =
    ...
    then
      Perf_script.collect ~perf:!perf ~perf_data:!perf_data ~binary:!binary
        ~code_bounds ~branch_sites
    else
      In_channel.with_open_text !perf_script_output
        (Perf_script.of_channel ~binary:!binary ~code_bounds ~branch_sites)
  in
  (* Symbolize each distinct address once. Sampled (leaf) addresses are looked
     ...
     byte back to land inside the call instruction. (Branch-stack sources and
     targets are instruction addresses and appear as leaves.) *)
  ...
  Hashtbl.iter
    ...
    samples.counts;
  (* Attribute each observed branch outcome to the pseudo-instrumentation labels
     of its machine edges, as recorded in the branch-label metadata. *)
  Hashtbl.iter
    (fun address ({ taken; fallthrough } : Perf_script.branch_counts) ->
      match Hashtbl.find_opt branch_labels address with
      | None -> ()
      | Some (site : Branch_labels.site) ->
        let add count hashes =
          P.Writer.add_hashed_stack writer ~hashes ~count ~max_depth:!max_depth
        in
        if Int64.compare taken 0L > 0 then List.iter (add taken) site.taken;
        if Int64.compare fallthrough 0L > 0
        then List.iter (add fallthrough) site.fallthrough)
    samples.branches;
  ...
  then
    Printf.eprintf
      "%s: %Ld samples in %d distinct stacks (%Ld at positions unknown to\n\
       the symbolizer); outcomes of %d of %d labelled branches; wrote %s\n"
      !binary samples.total
      (Hashtbl.length samples.counts)
      !unknown
      (Hashtbl.length samples.branches)
      (Hashtbl.length branch_labels)
      !output
