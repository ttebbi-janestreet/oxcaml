(* The little we need to know about the profiled executable's ELF file: its type
   ...
   not match their link-time addresses), the address range of its executable
   sections (to filter sampled branch addresses to the executable's own code)
   and its GNU build id (recorded in the profile as provenance). *)
...
let code_bounds t =
  Array.fold_left
    (fun acc (section : Owee_elf.section) ->
      if
        Int64.equal (Int64.logand section.sh_flags shf_execinstr) 0L
        || Int64.equal section.sh_size 0L
      then acc
      else
        let lo = section.sh_addr in
        let hi = Int64.add section.sh_addr section.sh_size in
        match acc with
        | None -> Some (lo, hi)
        | Some (acc_lo, acc_hi) ->
          Some
            ( (if Int64.compare lo acc_lo < 0 then lo else acc_lo),
              if Int64.compare hi acc_hi > 0 then hi else acc_hi ))
    None t.sections
