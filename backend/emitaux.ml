    ...
    | Some sourcefile ->
      ...
      let code_layout : Dwarf_state.code_layout =
        if
          ...
          (* FDO section sorting moves profile-hot functions into their own text
             sections while [code_begin] stays in the unit's default one, so
             label offsets relative to [code_begin] would cross sections. *)
