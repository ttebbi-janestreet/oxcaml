...
(** [label_recorded_branches ~from_pos ~to_pos ~recorded] inserts, for every
    recorded branch (conditional jump, call or jump) in the given output range
    that survived peephole optimization (which must have run already, so the
    labels cannot inhibit an optimization), a fresh label directly before the
    branch instruction, returning (label, datum) pairs in emission order.
    [recorded] pairs each branch's output position (see [current_output_pos])
    with a datum, in emission order; records whose instruction was optimized
    away are dropped. *)
val label_recorded_branches :
  from_pos:output_pos ->
  to_pos:output_pos ->
  recorded:(output_pos * 'a) list ->
  (Asm_targets.Asm_label.t * 'a) list
