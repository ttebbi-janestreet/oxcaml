...
let block_count profile (block : Cfg.basic_block) =
  ...
  let update (instr : _ Cfg.instruction) =
    let count = Source_position_profile.count_for_debuginfo profile instr.dbg in
...
(* Measured functions are sorted by hotness, approximating ocamlfdo's sorting of
   functions by execution count with power-of-two classes: a function whose
   hottest measured block has a count in [total/2^(k+1), total/2^k) goes to
   class k. The class names use the ".text.sorted." prefix so that the default
   GNU ld linker script (binutils >= 2.35) packs them via its
   "SORT(.text.sorted.<star>)" input section rule in name order - hottest class
   first - between ".text.hot" and ".text", with no linker script involvement;
   an older linker matches them with its plain ".text.*" rule and the layout
   degrades to input order. Functions the profile never saw stay in their usual
   section. *)
let hot_section_name klass = Printf.sprintf ".text.sorted.caml.%02d" klass

(* Two digits of zero-padding orders up to 100 classes; class 99 would mean a
   count 2^99 times smaller than the total, so the clamp is theoretical. *)
...
let assign_function_section ~dump profile cfg_with_layout counts =
  ...
  let section =
    (* The shared predicate keeps this in sync with the DWARF code-layout mode
       selection in [Emitaux.begin_dwarf]. *)
    if
          ...
          (fun (dst, labels) ->
            ( dst,
              List.fold_left
                (fun acc label ->
                  Int64.add acc
                    (Source_position_profile.count_for_branch_label profile
                       label))
                0L !labels ))
          !edges
      in
...
let reorder_blocks ~dump profile cfg_with_layout =
  ...
  then
    ...
    Cfg_with_layout.set_layout cfg_with_layout
      (DLL.of_list (List.concat_map (fun chain -> chain.blocks) chains))
