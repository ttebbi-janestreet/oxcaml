...
let label_recorded_branches ~from_pos ~to_pos ~recorded =
  (* [recorded] pairs the output position of each branch (conditional jump, call
     or jump) of interest with a datum, in emission order. Walk the emitted
     range; for every recorded branch whose instruction survived peephole
     optimization (this runs after it, so the labels cannot inhibit an
     optimization), insert a fresh label directly before it, giving the branch
     instruction's address. Records whose instruction disappeared are
     dropped. *)
  ...
  let rec walk cell pending acc =
    ...
    then List.rev acc
      ...
      | Some cell -> (
        ...
        | Ins (J _ | CALL _ | JMP _) -> (
          match extract cell pending with
          | None -> walk next pending acc
          | Some (datum, pending) ->
            let label = new_label () in
            DLL.insert_before cell (label_line label);
            walk next pending ((label, datum) :: acc))
